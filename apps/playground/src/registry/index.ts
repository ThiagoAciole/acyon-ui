...
function compareDefinitions(left: AnyComponentDefinition, right: AnyComponentDefinition) {
  const categoryOrder =
    CATEGORY_CONFIG[left.category].order - CATEGORY_CONFIG[right.category].order;
...
export const componentDefinitionsById = componentDefinitions.reduce<
...
export const sidebarGroups: SidebarGroup[] = sortBy(
  Object.entries(groupBy(componentDefinitions, (definition) => definition.category)).map(
    ([category, items]) => ({
